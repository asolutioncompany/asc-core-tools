...
- **Comments** – Disable comments and pings site-wide; optionally remove the Comments menu when there are no comments.
...
- **Social sharing** – Optional sharing bar (Facebook, Twitter, LinkedIn, Email, Copy link) on selected post types or via shortcode; requires Font Awesome when self-hosted.
- **Self-host Font Awesome** – Option to load Font Awesome assets from the plugin instead of a CDN.
...
- Logout links point directly to `wp-login.php` with the parameter and redirect to the home page after logout.
